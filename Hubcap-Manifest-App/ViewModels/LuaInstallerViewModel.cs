...
namespace HubcapManifestApp.ViewModels
{
    public partial class LuaInstallerViewModel : ObservableObject
    {
        ...
        /// <summary>
        /// Sets SelectedFiles, SelectedFilePath, SelectedFileName, HasFileSelected, and
        /// StatusMessage based on the validated file list and current tool mode.
        /// </summary>
        private void ApplyFileSelection(List<string> validFiles, ToolMode mode)
        {
            if (mode == ToolMode.SteamTools && validFiles.Count > 1)
            {
        ...
        private void ProcessDroppedFiles(string[] files)
        {
            ...
            ApplyFileSelection(validFiles, settings.Mode);
        }
                ...
                if (SelectedFilePath.EndsWith(".zip", StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    if (settings.Mode == ToolMode.DepotDownloader)
                    {
                        ...
                        var prep = await _depotInstallOrchestrator.PrepareDepotDataAsync(
                            SelectedFilePath, appId,
                            status => StatusMessage = status,
                            error => _notificationService.ShowError(error));
                        ...
                        var downloadResult = await _depotInstallOrchestrator.StartDownloadAsync(
                            prep, SelectedFilePath, depotDialog.SelectedDepotIds,
                            status => StatusMessage = status,
                            error => _notificationService.ShowError(error));
