...
namespace HubcapManifestApp.Services
{
    ...
    public class DepotFilterService
    {
        ...
        public List<string> GetAvailableLanguages(SteamCmdDepotData? steamCmdData, string appId, Dictionary<string, string>? depotKeys = null)
        {
            ...
            if ((hasBaseDepots || languages.Contains(SteamCmdApiService.DefaultLanguage)) && !languages.Contains(SteamCmdApiService.DefaultLanguage))
            {
                languages.Insert(0, SteamCmdApiService.DefaultLanguage);
            }
            ...
            bool skipOsFilter = false)
        {
            ...
            foreach (var kvp in depots)
            {
                ...
                if (depotInfo.Config != null)
                {
                    ...
                    if (!skipOsFilter && config.OsList == "macos")
                    {
                    ...
                    if (!skipOsFilter && config.OsList == "linux")
                    {
                    ...
                    if (language.ToLower() == SteamCmdApiService.DefaultLanguage)
                    {
                        // For English: include depots with no language OR explicitly marked as english
                        if (string.IsNullOrEmpty(depotLanguage) || depotLanguage.ToLower() == SteamCmdApiService.DefaultLanguage)
                        {
        ...
        /// <summary>
        /// Filters out non-Windows depots (macOS, Linux) from a depot list.
        /// Used when "All (Skip Filter)" is selected but OS filtering should still apply.
        /// </summary>
        public List<string> FilterNonWindowsDepots(SteamCmdDepotData? steamCmdData, List<string> depotIds, string appId)
        {
            if (steamCmdData?.Data == null || !steamCmdData.Data.ContainsKey(appId))
                return depotIds;
            ...
            {
                if (!depots.TryGetValue(depotId, out var depotInfo) || depotInfo.Config == null)
                {
                    filtered.Add(depotId);
                    continue;
                }

                var os = depotInfo.Config.OsList;
                if (os == "macos" || os == "linux")
                {
                    _logger.Debug($"OS filter: skipping depot {depotId} - {os} only");
                    continue;
                }
            ...
            _logger.Info($"OS filter: {depotIds.Count} -> {filtered.Count} depots (removed {depotIds.Count - filtered.Count} non-Windows)");
            return filtered;
        }
