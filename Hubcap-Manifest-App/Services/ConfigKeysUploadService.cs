...
using HubcapManifestApp.Tools.ConfigVdfKeyExtractor;
...
namespace HubcapManifestApp.Services
{
    public class ConfigKeysUploadService : IDisposable
    {
        ...
        public ConfigKeysUploadService(SettingsService settingsService, LoggerService loggerService, NotificationService notificationService, SteamService steamService, IHttpClientFactory httpClientFactory)
        {
        ...
        {
            if (!await _uploadGate.WaitAsync(0))
            {
            ...
            try
            {
                ...
                string configVdfPath = Path.Combine(steamPath, SteamPaths.ConfigDir, SteamPaths.ConfigVdf);
                ...
                _notificationService.ShowNotification("Config Keys Upload", $"Uploading {newKeys.Count} new depot keys to Hubcap...", NotificationType.Info);
        ...
        private async Task<HashSet<string>?> GetExistingDepotIdsAsync(string apiKey)
        {
            try
            {
                ...
                var response = await httpClient.GetAsync($"{ManifestApiService.BaseUrl}/depot-keys");
        ...
        private async Task<(bool Success, string Message)> UploadKeysFileAsync(string filePath, string apiKey)
        {
            try
            {
                ...
                var response = await httpClient.PostAsync($"{ManifestApiService.BaseUrl}/upload-machine-keys", form);
