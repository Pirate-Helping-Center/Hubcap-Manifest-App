    ...
    internal class Patcher
    {
        ...
        public PatchResult ApplyOfflineSetup()
        {
            ...
            try
            {
                ...
                if (cachePath == null)
                    return result.Fail("Payload cache not found. Launch Steam once so it creates the cache, then try again.");
        ...
        PatchEntry[] ResolveCorePatchOffsets(byte[] dll)
        {
            ...
            if (keyOffset < 0)
            {
                Log("  Core.dll: signature not found - not a recognized SteamTools version");
                return null;
            }
            Log($"  Core.dll: signature found at 0x{keyOffset:X}");
