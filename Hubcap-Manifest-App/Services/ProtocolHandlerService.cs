...
namespace HubcapManifestApp.Services
{
    public class ProtocolHandlerService
    {
        ...
        public async Task HandleProtocolAsync(string protocolPath)
        {
            ...
            {
                var appId = parts[2];
                if (!uint.TryParse(appId, out _)) return; // Reject non-numeric appIds to prevent path traversal
                await HandleDownloadAndInstall(appId, true);
            }
            else if (action == "download" && parts.Length >= 2)
            {
                var appId = parts[1];
                if (!uint.TryParse(appId, out _)) return;
        ...
        private async Task HandleDownload(string appId)
        {
            try
            {
                ...
                var manifest = new Manifest
                {
                    ...
                    IconUrl = "",
                    Size = 0,
                    DownloadUrl = $"{ManifestApiService.BaseUrl}/manifest/{appId}"
                };
