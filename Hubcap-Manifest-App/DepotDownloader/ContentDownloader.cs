...
namespace DepotDownloader
{
    ...
    public static class ContentDownloader
    {
        ...
        // Wrapper helper: ensure AppInfo for the given app has been fetched from PICS.
        // Returns true if the request completed (or was already cached), false on session
        // failure or exception. Callers can then use GetSteam3AppSection to read the data.
        public static async Task<bool> EnsureAppInfoAsync(uint appId)
        {
            if (steam3 == null) return false;
            try
            {
        ...
        private static async Task<DepotFilesData> ProcessDepotManifestAndFiles(CancellationTokenSource cts, DepotDownloadInfo depot, GlobalDownloadCounter downloadCounter)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    const int MaxManifestRetries = 10;
                    ...
                    do
                    {
                        ...
                        {
                            Console.WriteLine("Exceeded maximum retries ({0}) downloading depot manifest {1} {2}. Aborting.", MaxManifestRetries, depot.DepotId, depot.ManifestId);
                            cts.Cancel();
                            break;
                        }
            ...
            DepotManifest.ChunkData chunk)
        {
            ...
            finally
            {
                ...
                // Always decrement the chunk counter so resources are cleaned up even on cancellation
                remainingChunks = Interlocked.Decrement(ref fileStreamData.chunksToDownload);
