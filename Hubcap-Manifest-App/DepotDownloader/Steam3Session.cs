...
namespace DepotDownloader
{
    class Steam3Session
    {
        ...
        private async void ConnectedCallback(SteamClient.ConnectedCallback connected)
        {
            ...
            {
                Console.Error.WriteLine("Unhandled exception in ConnectedCallback: " + ex.Message);
                Abort(false);
            }
        }

        private async void DisconnectedCallback(SteamClient.DisconnectedCallback disconnected)
        {
            try
            {
            bDidDisconnect = true;
            ...
            else if (!bAborted)
            {
                ...
                await Task.Delay(1000 * connectionBackoff);
            ...
            {
                Console.Error.WriteLine("Unhandled exception in DisconnectedCallback: " + ex.Message);
            }
        }
